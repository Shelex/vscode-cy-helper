...
const attributes = {
    ...
    name: 'Belongs to <button>, <fieldset>, <form>, <iframe>, <input>, <map>, <meta>, <object>, <output>, <param>, <select>, <textarea>. Specifies the name of the element',
    placeholder:
    ...
    href: 'Belongs to <a>, <area>, <base>, <link>. Specifies the URL of the page the link goes to',
    accept: 'Belongs to <input>. Specifies the types of files that the server accepts (only for type="file")',
    checked:
    ...
    color: 'Belongs to Not supported in HTML 5. Specifies the text color of an element. Use CSS instead',
    contenteditable:
        'Specifies whether the content of an element is editable or not',
    draggable: 'Specifies whether an element is draggable or not',
    form: 'Belongs to <button>, <fieldset>, <input>, <label>, <meter>, <object>, <output>, <select>, <textarea>. Specifies the name of the form the element belongs to',
    required:
        'Belongs to <input>, <select>, <textarea>. Specifies that the element must be filled out before submitting the form',
    src: 'Belongs to <audio>, <embed>, <iframe>, <img>, <input>, <script>, <source>, <track>, <video>. Specifies the URL of the media file',
    selected:
        'Belongs to <option>. Specifies that an option should be pre-selected when the page loads',
    style: 'Specifies an inline CSS style for an element',
    type: 'Belongs to <a>, <button>, <embed>, <input>, <link>, <menu>, <object>, <script>, <source>, <style>. Specifies the type of element',
    value: 'Belongs to <button>, <input>, <li>, <option>, <meter>, <progress>, <param>. Specifies the value of the element',
    'accept-charset':
        'Belongs to <form>. Specifies the character encodings that are to be used for the form submission',
    accesskey: 'Specifies a shortcut key to activate/focus an element',
    action: 'Belongs to <form>. Specifies where to send the form-data when a form is submitted',
    align: 'Belongs to Not supported in HTML 5. Specifies the alignment according to surrounding elements. Use CSS instead',
    async: 'Belongs to <script>. Specifies that the script is executed asynchronously (only for external scripts)',
    autocomplete:
    ...
    border: 'Belongs to Not supported in HTML 5. Specifies the width of the border of an element. Use CSS instead',
    charset: 'Belongs to <meta>, <script>. Specifies the character encoding',
    cite: 'Belongs to <blockquote>, <del>, <ins>, <q>. Specifies a URL which explains the quote/deleted/inserted text',
    ...
    data: 'Belongs to <object>. Specifies the URL of the resource to be used by the object',
    ...
    defer: 'Belongs to <script>. Specifies that the script is executed when the page has finished parsing (only for external scripts)',
    ...
    for: 'Belongs to <label>, <output>. Specifies which form element(s) a label/calculation is bound to',
    formaction:
    ...
    height: 'Belongs to <canvas>, <embed>, <iframe>, <img>, <input>, <object>, <video>. Specifies the height of the element',
    high: 'Belongs to <meter>. Specifies the range that is considered to be a high value',
    hreflang:
    ...
    list: 'Belongs to <input>. Refers to a <datalist> element that contains pre-defined options for an <input> element',
    loop: 'Belongs to <audio>, <video>. Specifies that the audio/video will start over again, every time it is finished',
    low: 'Belongs to <meter>. Specifies the range that is considered to be a low value',
    ...
    media: 'Belongs to <a>, <area>, <link>, <source>, <style>. Specifies what media/device the linked document is optimized for',
    method: 'Belongs to <form>. Specifies the HTTP method to use when sending form-data',
    ...
    muted: 'Belongs to <video>, <audio>. Specifies that the audio output of the video should be muted',
    novalidate:
        'Belongs to <form>. Specifies that the form should not be validated when submitted',
    open: 'Belongs to <details>. Specifies that the details should be visible (open) to the user',
    optimum:
    ...
    poster: 'Belongs to <video>. Specifies an image to be shown while the video is downloading, or until the user hits the play button',
    preload:
    ...
    rel: 'Belongs to <a>, <area>, <form>, <link>. Specifies the relationship between the current document and the linked document',
    reversed:
        'Belongs to <ol>. Specifies that the list order should be descending (9,8,7...)',
    rows: 'Belongs to <textarea>. Specifies the visible number of lines in a text area',
    rowspan:
    ...
    scope: 'Belongs to <th>. Specifies whether a header cell is a header for a column, row, or group of columns or rows',
    shape: 'Belongs to <area>. Specifies the shape of the area',
    size: 'Belongs to <input>, <select>. Specifies the width, in characters (for <input>) or specifies the number of visible options (for <select>)',
    sizes: 'Belongs to <img>, <link>,<source>. Specifies the size of the linked resource',
    span: 'Belongs to <col>, <colgroup>. Specifies the number of columns to span',
    spellcheck:
        'Specifies whether the element is to have its spelling and grammar checked or not',
    srcdoc: 'Belongs to <iframe>. Specifies the HTML content of the page to show in the <iframe>',
    srclang:
        'Belongs to <track>. Specifies the language of the track text data (required if kind="subtitles")',
    srcset: 'Belongs to <img>, <source>. Specifies the URL of the image to use in different situations',
    start: 'Belongs to <ol>. Specifies the start value of an ordered list',
    step: 'Belongs to <input>. Specifies the legal number intervals for an input field',
    tabindex: 'Specifies the tabbing order of an element',
    target: 'Belongs to <a>, <area>, <base>, <form>. Specifies the target for where to open the linked document or where to submit the form',
    translate:
        'Specifies whether the content of an element should be translated or not',
    usemap: 'Belongs to <img>, <object>. Specifies an image as a client-side image map',
    width: 'Belongs to <canvas>, <embed>, <iframe>, <img>, <input>, <object>, <video>. Specifies the width of the element',
    wrap: 'Belongs to <textarea>. Specifies how the text in a text area is to be wrapped when submitted in a form'
};
